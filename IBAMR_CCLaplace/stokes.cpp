...
allocate_petsc_krylov_solver(const std::string& solver_object_name,
	SAMRAI::tbox::Pointer<SAMRAI::tbox::Database> solver_input_db,
	const std::string& solver_default_options_prefix)
{
	// This is an extension of petsc krylov linear solver that provides an implementation of the StaggeredStokesSolver
  ...
  //StaggeredStokesOperator implements a staggered-grid discretization of the incompressible Stokes operator
...
main(int argc, char* argv[])
{
	...
	{ // cleanup dynamically allocated objects prior to shutdown
	  ...
	  // file, and enable file logging.
		...
		PoissonSpecifications vc_vel_spec("vc_vel_spec");  // Set the PoissonSpecifications object used to specify
		// the coefficients in the incompressible Stokes operator
		vc_vel_spec.setCConstant(input_db->getDouble("C"));
		vc_vel_spec.setDPatchDataId(mu_nc_idx);
		StaggeredStokesOperator stokes_op("stokes_op");
		stokes_op.setVelocityPoissonSpecifications(vc_vel_spec);
		stokes_op.setPhysicalBcCoefs(u_bc_coefs, NULL);
		stokes_op.initializeOperatorState(u_vec, f_vec);
		stokes_op.setSolutionTime(0.0);
		...
		StaggeredStokesSolverManager* solver_manager = StaggeredStokesSolverManager::getManager();

		// Solver
		solver_manager->registerSolverFactoryFunction("PETSC_KRYLOV_SOLVER",
		...
		solver_manager->registerSolverFactoryFunction("STAGGERED_STOKES_LEVEL_RELAXATION_FAC_PRECONDITIONER",
		...
		solver_manager->registerSolverFactoryFunction("VC_VELOCITY_PETSC_LEVEL_SOLVER",
			// VCSCViscousPETScLevelSolver::allocate_solver);  // To solve on the coarsest level of multigrid
		Pointer<StaggeredStokesSolver> stokes_solver = solver_manager->allocateSolver(solver_type,
			"vc_velocity_solver",
			solver_db,
			"vc_velocity_",
			precond_type,
		...
		stokes_solver->setVelocityPoissonSpecifications(vc_vel_spec);
		stokes_solver->setPhysicalBcCoefs(u_bc_coefs);
		stokes_solver->setSolutionTime(0.0);
		...
		if (has_velocity_nullspace)
		{
			...
			Pointer<PETScKrylovLinearSolver> krylov_solver = stokes_solver;
			if (krylov_solver)
			{
		...
		// Solve L*u = f.
		u_vec.setToScalar(0.0);
		stokes_solver->initializeSolverState(u_vec, f_vec);
		stokes_solver->solveSystem(u_vec, f_vec);
